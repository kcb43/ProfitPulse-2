...
import { format, parseISO, subDays, startOfMonth, startOfYear, compareAsc } from 'date-fns';
import { Tabs, TabsList, TabsTrigger } from "@/components/ui/tabs";

export default function ProfitChart({ sales, range, onRangeChange }) {
  const { chartData, title } = React.useMemo(() => {
    if (!sales || sales.length === 0) {
      return { chartData: [], title: "Profit Trend" };
    }
    ...
    if (range === '14d') {
      dataTitle = "Profit Trend (Last 14 Days)";
      ...
      dataTitle = "Monthly Profit Trend";
    ...
    return { chartData: finalData, title: dataTitle };
