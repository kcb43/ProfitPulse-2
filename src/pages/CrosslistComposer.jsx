...
// Mercari-specific brands - extracted from Mercari's brand dropdown
// To update: Run scripts/extract-mercari-brands-console.js in browser console on Mercari sell page
// Currently only includes brands starting with "A" - other letters will be added incrementally
const MERCARI_BRANDS = [
  "A Bathing Ape",
  "A Beautiful Soul",
  "A BETTER FEELING",
  "A BETTER MISTAKE",
...
export default function CrosslistComposer() {
  ...
  // For Mercari: Only applies brands that exist in Mercari's list, but if there is a similar name, it will auto-select it.
  // The user can change it if it's not what they want anyway.
  useEffect(() => {
    ...
    if (exactMatch) {
      ...
      if (similarMatch) {
        ...
        const mercariMatch = findSimilarBrand(generalBrand, MERCARI_BRANDS);
        if (!mercariMatch && mercariForm.brand && mercariForm.brand === generalBrand && !MERCARI_BRANDS.includes(mercariForm.brand) && !customBrands.includes(mercariForm.brand)) {
          handleMarketplaceChange("mercari", "brand", "");
        }
      }
    }
                    ...
                    <PopoverTrigger asChild>
                      ...
                      >
                        {mercariForm.brand || generalForm.brand
                          ? (MERCARI_BRANDS.find((brand) => brand === (mercariForm.brand || generalForm.brand)) || 
                             (() => {
                               // Check if general form brand matches any Mercari brand
                               const matched = findSimilarBrand(generalForm.brand, MERCARI_BRANDS);
                               return matched || (mercariForm.brand || generalForm.brand);
                             })())
                          : "Search brand..."}
                        <ArrowRight className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                      </Button>
                      ...
                      <Command>
                        ...
                        <CommandList>
                          ...
                          <CommandGroup>
                            ...
                            {customBrands
                              .filter(brand => {
                                // Only show custom brands that match a Mercari brand
                                const matched = findSimilarBrand(brand, MERCARI_BRANDS);
                                return matched !== null;
                              })
                              .map((brand) => {
                                const matchedBrand = findSimilarBrand(brand, MERCARI_BRANDS);
                                return (
                                  <CommandItem
                                    key={`custom-${brand}`}
                                  ...
                                  >
                                    <Check
                                      className={cn(
                                        "mr-2 h-4 w-4",
                                        mercariForm.brand === matchedBrand ? "opacity-100" : "opacity-0"
                                      )}
                                    />
                                    <span>{matchedBrand}</span>
                                    <span className="text-xs text-muted-foreground ml-2">⭐ Custom</span>
                                  </CommandItem>
                                );
                              })}
                            {customBrands.filter(brand => findSimilarBrand(brand, MERCARI_BRANDS) !== null).length > 0 && (
                              <div className="border-t my-1" />
                            )}
                            {/* Mercari-specific brands */}
                            {MERCARI_BRANDS.map((brand) => (
                              <CommandItem
                                key={brand}
                                value={brand}
                                onSelect={() => {
